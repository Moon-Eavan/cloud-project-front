...
export default function When2MeetScheduler({ groupName, groupMembers, onAddEvent, onTimeSelected }: Props) {
  ...
  const [currentUserSlots, setCurrentUserSlots] = useState<Map<string, string>>(new Map());
  ...
  const startScheduling = () => {
    if (selectedMembers.length > 0) {
      // 선택된 멤버들에 대한 예시 일정 데이터 생성
      const exampleParticipants: Participant[] = selectedMembers.map((member, idx) => {
        // 각 멤버마다 랜덤한 일정 생성
        const slots = new Set<string>();
        const randomSlotCount = Math.floor(Math.random() * 5) + 3;
        for (let i = 0; i < randomSlotCount; i++) {
          const randomDay = formatDate(dateRange[Math.floor(Math.random() * dateRange.length)]);
          const randomHour = hours[Math.floor(Math.random() * hours.length)];
          slots.add(`${randomDay}-${randomHour}`);
        }
        return {
          name: member,
          slots,
  ...
  // 현재 사용자가 이 시간대를 선택했는지 확인
  const isMySchedule = (day: string, hour: number) => {
    const key = getSlotKey(day, hour);
    return currentUserSlots.has(key);
  };
  ...
  const handleMouseEnter = (day: string, hour: number) => {
    ...
    // 내 일정이나 다른 사람 일정이 있으면 선택 불가
    if (isDragging && !isMine && !hasOthers) {
      toggleSlot(day, hour);
    }
  };
  ...
  const getSlotContent = (day: string, hour: number) => {
    ...
    if (isMine) {
      const title = currentUserSlots.get(key);
      return (
            ...
            <div className="grid grid-cols-2 gap-3">
              {groupMembers.map((member) => (
                ...
                >
                  ...
                  />
                  <label className="cursor-pointer flex-1">{member}</label>
                </div>
              ))}
            </div>
          </div>
